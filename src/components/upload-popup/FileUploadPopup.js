...
const FileUploadPopup = ({ onClose, onUpload, department, action , subCategory }) => {
  ...
  const handleSubmit = async () => {
    ...
    if (selectedFile) {
      try {
        ...
        const response = await axios.post(`http://localhost:8009/${action}/${department}/${subCategory}`, formData, {
          headers: {
      ...
      >
        ...
        <label className="drop-area" htmlFor="file-input">
          {selectedFile ? selectedFile.name : (
            <React.Fragment>
              <CloudUploadIcon style={{height:"6vw", width:"20vw"}} /> Drag & Drop to Upload
            </React.Fragment>
          )}
